...
export default class Logger {
	...
	private static saveToFile(obj: ILogObject) {
		...
		fs.appendFileSync(`logs/${current}.log`, `${obj.logLevel.toUpperCase()} [${obj.loggerName ?? "Unknown"}${!obj.filePath ? "" : ` ${obj.filePath}`}${!obj.typeName || !obj.functionName ? "" : ` ${obj.typeName}.${obj.functionName}`}] ${obj.argumentsArray.join(" ")}\n`);
	}
